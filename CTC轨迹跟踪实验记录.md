...
最初是想做个圆，但是注意到模型 `mdl` 中的轨迹生成模块使用顶点坐标矩阵生成轨迹。以正50边形为例，轨迹通过如下方式生成
...
最初设定的轨迹是一个内接于单位圆的正50边形，但在模拟过程中出现奇异点，此后chang's猜测是由于==轨迹上的某些位置该机械臂难以到达==，或者由于==CTC-PID控制的方案本身存在局限，使得对关节角度规划不合理，导致末端运动受限==。
# 六、下一步实验计划
